...
#### 루트 도로 geometry (`assets/data/route_geometry.json`)
...
> | **프론트엔드** | `activities.html` + `assets/js/activities.js` (JS 내부 인라인 데이터, 외부 JSON 없음) |
...
> **현재 상태**: ⬜ Phase 5 미시작. Phase 4 일정 확정 후 진행 예정. 단, 숙소 평가(CRITIC_LODGING.md 기반 77개 숙소 채점 — 기본평가 44 + 투자평가 33, 비선정 18개. 상세 수치는 `종합비교.md` SSOT 참조)와 식당 평가(CRITIC_DINING.md 기반 140개+ 리서치 → 126개 CRITIC 평가)는 완료. 최종 루트 선택 후 일정 통합 검증 필요.
...
│   ├── js/activities.js        #   액티비티 페이지 인터랙션 (JS 내부 인라인 데이터, 10개 지역 109개)
│   └── data/
│       ├── place_data.json     #   프론트엔드용 장소 데이터 (자동 생성)
│       ├── route_data.json     #   루트 비교용 경유지·좌표 데이터
│       └── route_geometry.json #   루트 도로 geometry (OSRM 자동 생성, encoded polyline)
│
