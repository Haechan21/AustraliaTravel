...
> **총 이동거리**: 2,379km (Day 1 직행 862km 포함, Day 2~6 관광 구간 1,517km)
...
> **4조 대비 핵심 개선**: Day 5(540km) → Day 3(~440km) + Day 4(~50km 풀데이) + Day 5(328km)으로 재분배. Day 4 포트스테판스 풀데이 여유일 신설
...
| 최대 이동거리 (Day 1 제외) | 540km (Day 5) | **~440km** (Day 3) |
...
### Day 3 — 5/26(화): 콥스하버 → 도리고 → 포트맥쿼리(경유) → 포트스테판스 (~440km, 빡센 날)

- **구간**: 콥스하버 → 도리고 국립공원 → 포트맥쿼리(경유) → 포트스테판스
- **이동거리**: ~440km (도리고 우회 + 포맥→포트스 포함)
...
| 10:00~10:20 | **포레스트 스카이 피어** (Dorrigo Skywalk) | **A** (79.2) | 열대우림 캐노피 위 70m 전망대. 우천 시에도 관람 가능 |
...
**방문 장소**: S 1곳(룩앳미나우), A 3곳 (스카이피어, 크리스탈샤워, 댕거폭포)
...
> **v6 변경점**: 기존 v5는 Day 3에 콥스→도리고→포맥(387km, 포맥 숙박)이었으나, v6에서는 포맥을 경유지로 바꾸고 포트스테판스까지 당일 이동(~440km). 태킹포인트 등대(A, 78.2)를 포기하는 대신, Day 4를 포트스테판스 풀데이 여유일로 확보. 빡센 날을 하루로 몰아넣고 다음 날 여유를 얻는 트레이드오프.
...
| 3 | 5/26 | GC→콥스하버 | 335km | 콥스→도리고→포맥→포트스 | ~440km | +105km |
...
| **합계** | | | **2,567km** | | **2,379km** | +83km |

### 핵심 개선 하이라이트

| 지표 | 4조 | 9조 | 개선 |
|------|-----|-----|------|
| Day 1 제외 최대 이동거리 | **540km** (Day 5) | **~440km** (Day 3) | 100km 감소 |
| 400km 초과일 (Day 1 제외) | **1일** (Day 5) | **1일** (Day 3, ~440km) | 규모 축소 |
...
| Day 3 (5/26 화) | 콥스→도리고→포맥(경유)→포트스 | ~440km | 룩 앳 미 나우 | 해안 트레일 + 열대우림 폭포 | 빡센 |
...
| 항목 | 수치 |
|------|------|
| 총 이동거리 | 2,379km (Day 1 직행 862km 포함) |
...
1. **'죽음의 행군' 해소 + Day 4 여유일 신설**: 4조 Day 5의 540km를 Day 3(~440km) + Day 4(~50km 풀데이) + Day 5(328km)으로 재분배. Day 3이 빡센 날(~440km)이 되지만, 그 보상으로 Day 4가 이동거리 50km의 완전한 여유일로 변모. 포트스테판스에서 시간 압박 없이 샌드보딩·토마리 헤드를 즐길 수 있음.
...
3. **Day 2~3 연속 이동 부담**: Day 2(338km, 이동+관광) → Day 3(~440km, 빡센)으로 이틀 연속 장거리. 단, Day 4가 풀데이 여유일(~50km)이므로 "빡센→여유" 교차 패턴은 확보. 태킹포인트 등대(A, 78.2) 포기로 A등급 1곳 감소(15→14곳).
...
**장점:**
- S등급 6곳 올클리어를 유지하면서 Day 4 풀데이 여유일(~50km) 신설. 빡센 날(Day 3, ~440km) → 여유일(Day 4) 교차 리듬 확보
...
**장점:**
- 4조의 최대 약점(Day 5 540km)을 재분배하여 해결. Day 3(~440km)이 빡세지만, Day 4 풀데이(~50km)로 즉시 회복
- Day 3 ~440km는 400km 초과이나, 다음 날이 여유일이므로 피로 누적 리스크 관리 가능
...
- Day 3 콥스→도리고→포맥→포트스(~440km)가 상당히 빡빡. 07:00 출발 필수이며, 포트스 도착이 17:25로 야간 약 25분 포함. 태킹포인트 등대(A, 78.2)를 포기하는 트레이드오프
...
### v6 재구성 (CRITIC_ROUTE, Day 3-4 재배치)
...
> **v5 → v6 변경 사항** (+1.5점):
> 1. Day 3: 콥스→도리고→포맥 숙박(387km) → 콥스→도리고→포맥(경유)→포트스(~440km)로 변경. 태킹포인트 등대(A, 78.2) 생략
> 2. Day 4: 포맥→포트스 이동+관광(374km) → 포트스테판스 풀데이 여유일(~50km)로 변경. 샌드보딩+토마리+보기홀을 시간 여유롭게 배치
> 3. A등급 15→14곳(태킹포인트 탈락), 골든아워 4→3회(태킹포인트 일몰 탈락)
> 4. Day 4가 이 여행의 유일한 '완전 여유 날'(~50km)이 됨 — 피로 회복 + 체험 충실도 향상
